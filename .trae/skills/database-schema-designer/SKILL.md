...
description: Design robust, scalable database schemas for SQL and NoSQL databases. Provides normalization guidelines, indexing strategies, migration patterns, constraint design, and performance optimization. Ensures data integrity, query performance, and maintainable data models. 及时上网搜索学习新技术和方法。
version: 2.0.0
license: MIT
---
...
flowchart TB
    ...
    INPUT -->|需求阶段| RF["requirements-fractal<br/>数据需求"]
    INPUT -->|设计阶段| FD["fractal-designer<br/>数据库设计"]

    RF --> ANALYZE["分析数据模型<br/>实体/关系/约束"]
    FD --> ANALYZE

    ANALYZE --> DESIGN["Schema设计<br/>DDL+索引+迁移"]
    DESIGN --> WEB{"需搜索方案?"}
    WEB →|是| SEARCH["WebSearch<br/>最新数据库模式/工具"]
    SEARCH --> OUTPUT
    WEB →|否| OUTPUT(["输出: DDL+迁移脚本<br/>+索引策略"])

    OUTPUT --> DEV["fullstack-developer<br/>数据库实现"]
    OUTPUT --> FRR["full-review-repair-fractal<br/>Schema审查"]
...
- **每次操作记录时间戳**，涉及新数据库技术/ORM时使用 WebSearch
...
```
[fractal-designer / fullstack-developer] → [database-schema-designer] → [fullstack-developer 实现DB]
```

**本角色**：生产级数据库Schema设计专家，提供SQL/NoSQL的规范化、索引、迁移和性能优化指导。

### 上游输入

| 上游技能 | 输入数据 | 使用方式 |
|----------|----------|----------|
| **fractal-designer** | 数据结构设计文档（来自标准文档集） | 驱动实体识别、关系建模、访问模式分析 |
| **fullstack-developer** | 数据库实现需求 | 提供Schema设计和迁移脚本 |

### 下游输出

| 输出内容 | 消费者 | 使用方式 |
|----------|--------|----------|
| SQL/NoSQL Schema DDL | fullstack-developer | 直接用于创建数据库表/集合 |
| 迁移脚本（up+down） | task-scheduler-fractal | 作为DB相关任务的一部分 |
| 索引优化策略 | full-review-repair-fractal | 作为后端审查的参考基准 |

### 协作协议

#### ← fractal-designer / fullstack-developer
- **调用时机**：需要设计或实现数据存储层时
- **输入**：实体列表、关系描述、访问模式（读写比例、查询模式）
- **输出**：完整DDL + 迁移脚本 + 索引策略
- **关键对接点**：
  - 领域模型 → 表/集合设计
  - 业务关系 → 外键/引用设计
...
| 协作技能 | 关系 | 说明 |
|----------|------|------|
| api-design-principles | 并行参考 | API资源模型应与DB Schema对齐 |
| cache-components | 后续优化 | 查询结果缓存可结合Cache Components使用 |
| code-reviewer | 下游审查 | DB迁移代码由code-reviewer审查安全性 |
