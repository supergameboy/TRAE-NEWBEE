...
description: This skill is for interface design — dashboards, admin panels, apps, tools, and interactive products. NOT for marketing design (landing pages, marketing sites, campaigns). 及时上网搜索学习新技术和方法。
version: 2.0.0
---
...
flowchart TB
    ...
    INPUT -->|设计阶段| FD["fractal-designer<br/>UI/UX环节"]
    INPUT -->|用户直接| DESC["界面描述/参考/偏好"]

    FD --> ANALYZE["分析需求<br/>布局+交互+数据"]
    DESC --> ANALYZE

    ANALYZE --> DESIGN["设计方案<br/>Design System定义"]
    DESIGN --> WEB{"需搜索灵感?"}
    WEB -->|是| SEARCH["WebSearch<br/>最新UI模式/趋势"]
    SEARCH --> OUTPUT
    WEB -->|否| OUTPUT(["输出: 界面设计方案<br/>+Design Tokens"])

    OUTPUT --> FED["frontend-design<br/>视觉规范转化"]
    OUTPUT --> FUT["frontend-ui-test<br/>效果验证"]
...
- **每次操作记录时间戳**，涉及新UI模式时使用 WebSearch
...
```
[fractal-designer UI/UX设计] → [interface-design] → [frontend-design 实现界面]
[需求中的界面要求] ────────→                  → [frontend-ui-test 验证效果]
```

**本角色**：界面设计专项技能，专注于 Dashboard、管理面板、应用工具等界面的设计系统和交互模式。不适用于营销型落地页。

### 上游输入

| 上游技能 | 输入数据 | 使用方式 |
|----------|----------|----------|
| **fractal-designer** | UI/UX设计文档 + 交互流程 | 驱动界面布局和组件选型 |
| **requirements-fractal** | 用户故事 + 界面需求 | 确保界面满足用户场景 |
| 用户直接输入 | 界面描述/参考/偏好 | 直接启动界面设计 |

### 下游输出

| 输出内容 | 消费者 | 使用方式 |
|----------|--------|----------|
| 界面设计方案（含Design Tokens） | frontend-design | 作为前端实现的视觉规范 |
| 组件规格说明 | fullstack-developer | 指导组件开发 |
| 设计系统定义 | frontend-ui-test | 界面验证的参考基准 |
| 交互规范 | full-review-repair-fractal | UX审查依据 |

### 协作协议

#### ← fractal-designer
- **调用时机**：设计阶段的UI/UX设计环节
- **数据接收**：用户交互设计、数据流转设计、L2-L3组件级设计
- **输出**：完整的界面设计方案（布局、配色、字体、间距、动效）

#### → frontend-design
- **核心对接**：传递 Design System 定义（颜色/字体/间距/阴影/圆角等）
- **交付物**：界面原型描述 + CSS变量体系 + 组件规格

#### → frontend-ui-test
- **验证依据**：界面设计稿作为测试对比基准
- **检查项**：布局还原度、视觉一致性、响应式适配

### 参考资源集成

| 资源文件 | 内容 | 使用时机 |
|----------|------|----------|
| `references/principles.md` | 设计原则、代码示例、暗色模式 | 设计时参考 |
| `references/validation.md` | 内存管理、更新时机 | 实现时参考 |
| `references/critique.md` | 构建后评审协议 | 完成后自评 |
| `references/example.md` | 完整示例 | 学习参考 |

### 协作约束

- **非营销型**：本技能专注工具/仪表盘类界面，不用于营销落地页
- **系统思维**：从整体设计系统角度思考，而非单页面
- **可操作性**：设计方案必须可直接转化为前端代码
